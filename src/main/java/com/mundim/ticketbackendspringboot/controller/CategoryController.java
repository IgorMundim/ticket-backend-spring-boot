...
public class CategoryController {
    ...
    public ResponseEntity<CategoryResponseDto> create(@RequestBody @Valid CategoryRequestDto categoryDto){
        CategoryResponseDto responseDto = iCategoryService.createCategory(categoryDto);
        return ResponseEntity.status(201).body(responseDto);
    }
    @GetMapping("/{id}")
    public ResponseEntity<CategoryResponseDto> getById(@PathVariable Long id){
        CategoryResponseDto responseDto = iCategoryService.fetchCategory(id);
    ...
    public ResponseEntity<CategoryResponseDto> update(@RequestBody @Valid CategoryRequestDto eventDto, @PathVariable Long id){
        CategoryResponseDto responseDto = iCategoryService.updateCategory(eventDto, id);
        return ResponseEntity.status(200).body(responseDto);
    }
